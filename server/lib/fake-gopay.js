...
export async function generateFakeGopay(data = {}) {
    ...
    ctx.fillText(
        Number(data.koin).toLocaleString("id-ID"),
        pos.koin.x,
        pos.koin.y
    );
    const koinAngkaW = ctx.measureText(
        Number(data.koin).toLocaleString("id-ID")
    ).width;
    ...
    const textStartX =
        pos.pill.x + pill.paddingLeft + icon.report.w + pill.gapIconText;
